...
export default function DluhyPage() {
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({
    ...
    background: active ? '#e53e3e' : 'transparent', color: active ? 'white' : 'var(--muted)',
  })
          ...
          <tbody>
            {shown.length === 0 ? (
              ...
              <tr key={d.id} style={{ borderBottom: '1px solid var(--border)' }}>
                <td style={{ padding: '12px 16px', fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{d.komu_kdo}</td>
                ...
                <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--muted)' }}>{d.popis || '—'}</td>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => deleteDebt(d.id)} style={{ background: 'transparent', border: 'none', color: '#e53e3e', cursor: 'pointer', padding: 4 }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Přidat dluh">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
            ...
            <button onClick={save} disabled={saving || !form.komu_kdo || !form.castka || !form.datum} style={{ background: '#e53e3e', border: 'none', borderRadius: 8, padding: '10px 16px', color: 'white', cursor: 'pointer', fontSize: 14, fontWeight: 600, opacity: saving ? 0.6 : 1 }}>
