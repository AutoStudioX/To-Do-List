...
- **Appka se nepřenačítá sama.** Jediná výjimka je `components/StaleReloader.tsx`: dokument běžící bez obnovení déle než 8 hodin se načte znovu ve chvíli, kdy se uživatel VRACÍ na záložku (`visibilitychange` → visible). Žádný časovač, žádné hlídání nové verze, žádná lišta s upozorněním — nová verze se normálně načte tím, že uživatel sám obnoví stránku. Předchůdce (`UpdateReloader`) porovnával hashe skriptů a volal `location.reload()` kdykoli, i uprostřed psaní; rozepsaný text tím mizel.
- **Rozepsaný text nesmí zmizet — ani při přenačtení.** Každý formulář s delším psaním drží draft v `localStorage` přes `useDraft()` (klíč nese identitu záznamu, jinak se text vylije do cizího formuláře). Draft mizí po uložení a po „Zahodit", ne při zavření okna. A co přepisuje stav ze serveru, musí respektovat rozepsané: `load()` na Habits nepřepíše den, do kterého se právě píše.
